...
public class WorkerBusinessService
    : BaseService<Worker, WorkerFilterOptions, WorkerApiRequestDto, WorkerApiRequestDto>,
        IWorkerBusinessService
{
    private readonly IWorkerRepository _workerRepository;

    public WorkerBusinessService(IWorkerRepository workerRepository)
    ...
    protected override ValueTask<Result> ValidateForCreateAsync(WorkerApiRequestDto createDto)
    {
        ...
        if (createDto.Name.Length < 1)
            return ValueTask.FromResult(
                Result.Failure("Worker name must be at least 1 character long.")
            );

        if (createDto.Name.Length > 100)
            return ValueTask.FromResult(
                Result.Failure("Worker name cannot exceed 100 characters.")
            );
        ...
        if (!string.IsNullOrWhiteSpace(createDto.Email))
        {
            if (!IsValidEmail(createDto.Email))
                return ValueTask.FromResult(
                    Result.Failure("Email must be in basic format: user@domain.extension")
                );

            if (createDto.Email.Length > 254)
                return ValueTask.FromResult(
                    Result.Failure("Email address cannot exceed 254 characters.")
                );
        }
        ...
        if (!string.IsNullOrWhiteSpace(createDto.PhoneNumber))
        {
            if (!IsValidPhoneNumber(createDto.PhoneNumber))
                return ValueTask.FromResult(
                    Result.Failure("Phone number must contain at least 10 digits.")
                );

            if (createDto.PhoneNumber.Length > 20)
                return ValueTask.FromResult(
                    Result.Failure("Phone number cannot exceed 20 characters.")
                );
        }
        ...
        if (
            ...
            && string.IsNullOrWhiteSpace(createDto.PhoneNumber)
        )
            return ValueTask.FromResult(
                Result.Failure("At least one contact method (email or phone) is required.")
            );
    ...
    protected override async Task<Result> ValidateForDeleteAsync(int id)
    {
        ...
        if (hasShifts)
        {
            return Result.Failure(
                $"Cannot delete worker '{worker.Name}' because they have associated shifts. Please reassign or delete the shifts first."
            );
        }
