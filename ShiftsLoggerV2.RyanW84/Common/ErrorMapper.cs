...
public static class ErrorMapper
{
    ...
    public static (HttpStatusCode Status, string Message) Map(Exception ex)
    {
        if (ex is ArgumentNullException)
            return (HttpStatusCode.BadRequest, "A required value was null.");
        if (ex is ArgumentException)
            return (HttpStatusCode.BadRequest, "Invalid argument provided.");
        if (ex is InvalidOperationException)
            return (HttpStatusCode.Conflict, "Invalid operation in current state.");
        if (ex is DbUpdateException)
            return (HttpStatusCode.Conflict, "Database update failed.");
        if (ex is TimeoutException)
            return (HttpStatusCode.RequestTimeout, "The operation timed out.");
