...
public abstract class BaseController : ControllerBase
{
    ...
    /// </summary>
    protected ActionResult<ApiResponseDto<string>> HandleNonGenericResult(
        Result result,
    ...
    /// </summary>
    protected ActionResult<ApiResponseDto<T>> HandleResult<T>(
        Result<T> result,
        string successMessage
    )
    {
        if (!result.IsSuccess)
        {
            ...
            if (
                ...
                && result.Data is IEnumerable<object> emptyEnumerable
                && !emptyEnumerable.Any()
            )
            {
                return Ok(
        ...
        return Ok(
            new ApiResponseDto<T>
            {
                ...
                Message = successMessage,
                Data = result.Data,
                TotalCount = result.Data is IEnumerable<object> enumerable
                    ? enumerable.Count()
    ...
    /// </summary>
    protected ActionResult<PaginatedApiResponseDto<T>> HandlePaginatedResult<T>(
        Result<T> result,
        Models.FilterOptions.BaseFilterOptions filterOptions,
        string successMessage
    )
    {
        if (!result.IsSuccess)
    ...
    /// </summary>
    protected ActionResult<ApiResponseDto<T>> Success<T>(
        T data,
    ...
    )
    {
        return Ok(
            new ApiResponseDto<T>
            {
                ...
                Message = message,
                Data = data,
                TotalCount = data is IEnumerable<object> enumerable
                    ? enumerable.Count()
                    : totalCount,
            }
        );
    ...
    /// </summary>
    protected ActionResult<ApiResponseDto<string>> Success(
        string message = "Operation completed successfully"
    )
    {
        return Ok(
    ...
    /// </summary>
    protected ActionResult<ApiResponseDto<T>> Error<T>(
        string message,
    ...
    /// </summary>
    protected ActionResult<ApiResponseDto<string>> Error(
        string message,
        HttpStatusCode statusCode = HttpStatusCode.BadRequest
    )
    {
